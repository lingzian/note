...
import经过webpack打包以后变成一些Map对象，key为模块路径，value为模块的可执行函数；就是webpack定义的__webpack_require__函数，负责实际的模块加载并执行这些模块内容，返回执行结果，其实就是读取Map对象，然后执行相应的函数；
...
`打包优化`
- 打包体积：1、比如import了一个service 这个service里面的有import了很多不需要启动时就加载的库  但是没用延迟加载 （await import() ） 就会导致很多依赖库都被抽到最开始的libs 和app.js里面
2、analyzer插件来分析打包构成，发现有一些包被重复的打包了多次，这样会让构建出来的包格外的臃肿。这主要是因为我们往往引用了很多的第三方包，而很多工具类的库也会被别的包间接的依赖，所以就导致了重复打包的现象
3 资源压缩
4 tree-shaking
5 split-chunk(把node_module里面的插件分别打包成一个chunk。 也可以自定义设置什么东西需要打包成一个chunk，可以试引用了两次以上的东西打包成一个chunk)
...
`如何提高webpack的构建速度？`
优化 loader 配置 (include excloude 去掉不需要检查的文件范围)
使用 cache-loader 以将结果缓存到磁盘里，显著提升二次构建速度
启动多线程
...
...
- Rollup
  是一款 ES Modules 打包器，
  是一个 JavaScript 模块打包器，可以将小块代码编译成大块复杂的代码，
